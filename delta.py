...
class AWSSecretsManager:
    ...
    def __init__(self):
        ...
        if RRR_AVAILABLE:
            ...
            self.fetch_error = "❌ RRR library not available. Running in degraded mode without AWS secrets."
            self.logger.warning(self.fetch_error)
    ...
    def _fetch_secrets_from_aws(self):
        ...
        try:
            ...
            self.fetch_error = f"❌ Failed to fetch secrets from AWS: {e}"
            self.available = False
            self.logger.error(self.fetch_error, exc_info=True)
    
    def get_token_payload(self) -> Dict[str, str]:
        """Get OAuth token request payload with credentials from AWS"""
        if not self.available:
            self.logger.warning("AWS secrets not available; returning empty token payload")
            return {}
        if not self.client_id or not self.client_secret:
            self.logger.warning("OAuth credentials not loaded from AWS")
            return {}
...
try:
    ...
    if aws_secrets.available:
        logger.info("✅ AWS Secrets Manager initialized successfully")
    else:
        logger.warning("⚠️ AWS Secrets Manager in degraded mode - API calls that need AWS credentials will be unavailable")
except Exception as e:
    logger.error(f"❌ Unexpected error initializing AWS Secrets Manager: {e}", exc_info=True)
    class _FallbackAWSSecrets:
        """Lightweight fallback to keep server running when AWS init fails"""
        available = False
        client_id = None
        client_secret = None
        grant_type = HealthMCPConfig.GRANT_TYPE
        cert_path = None
        headers_with_secrets: Dict[str, str] = {}
        body_with_secrets: Dict[str, str] = {}
        params_with_secrets: Dict[str, str] = {}
        def get_token_payload(self) -> Dict[str, str]:
            return {}
        def get_token_headers(self) -> Dict[str, str]:
            return {"Content-Type": "application/x-www-form-urlencoded"}
        ...
        def get_verify_value(self) -> bool:
            return True
    aws_secrets = _FallbackAWSSecrets()
    logger.warning("⚠️ Using fallback AWS secrets stub - token-protected APIs will respond with degraded status")
...
def get_access_token_sync() -> str | None:
    ...
    if not aws_secrets.available:
        logger.warning("AWS secrets unavailable; skipping token request")
        return None
    try:
...
async def async_get_token() -> Dict[str, Any]:
    ...
    if not aws_secrets.available:
        return {"status_code": 503, "error": "AWS secrets unavailable; token service disabled"}
...
async def async_submit_request(user: UserInput, url: str) -> Dict[str, Any]:
    ...
    if not aws_secrets.available:
        return {"status_code": 503, "error": "AWS secrets unavailable; request not sent", "url": url}
...
async def async_mcid_search(user: UserInput) -> Dict[str, Any]:
    """Search MCID service"""
    if not aws_secrets.available:
        return {"status_code": 503, "error": "AWS secrets unavailable; MCID search disabled"}
    token = await asyncio.to_thread(get_access_token_sync)
    if not token:
        return {"status_code": 500, "error": "Access token not found"}
