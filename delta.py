...
def main():
    ...
    if st.session_state['show_response']:
        ...
        print(cortex_query_ip)
        query_ip = cortex_query_ip
        
        # Extract the query using the function
        query_ip = extract_cypher_query(query_ip)
        ...
        if query_ip:
            ...
            if technique_type == "Total Rows":
                ...
                st.write("Results are shown in Browser")
            ...
            st.warning("⚠️ Could not extract Cypher query from the response.")
